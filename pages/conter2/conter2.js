...
Page({
  ...
  listenerButtonPlay: function (e) {
    ...
    var that = this;
    bk_id = wx.getStorageSync("bk_id")
    var status = 2
    if (bk_id != e.currentTarget.dataset.index) {
      status = 1

      backgroundAudioManager.stop();


      wx.setStorageSync("bk_id", e.currentTarget.dataset.index);
      if (wx.getStorageSync("item")) {
        if (bk_id) {
          wx.request({
            url: 'https://www.chuang1.cn/api/bookrecord',
            method: "get",
            data: { 'bk_id': bk_id, "tel": tel, "time": wx.getStorageSync("item"), "type": 1 },
          ...
          wx.request({
            url: 'https://www.chuang1.cn/api/bookrecordd',
            method: "get",
            data: { 'bk_id': bk_id, "tel": tel, "time": wx.getStorageSync("item") },
            success: function (res) {
    ...
    wx.request({
      url: 'https://www.chuang1.cn/api/bookread',
      method: "get",
      data: { bk_id: that.data.bk_id, tel: tel },
      success: function (res) {
  ...
  listenerButtonPause: function () {
    ...
    var bk_id = wx.getStorageSync("bk_id")
    var tel = wx.getStorageSync("phone")
    ...
    wx.request({
      url: 'https://www.chuang1.cn/api/bookrecord',
      method: "get",
      data: { 'bk_id': bk_id, "tel": tel, "time": Math.floor(backgroundAudioManager.currentTime), "type": 1 },
      success: function (res) {
    ...
    wx.request({
      url: 'https://www.chuang1.cn/api/bookrecordd',
      method: "get",
      data: { 'bk_id': bk_id, "tel": tel, "time": Math.floor(backgroundAudioManager.currentTime) },
      success: function (res) {
  ...
  changeSlide: function (e) {
    ...
    if (value == 0) {
      ...
      if (e.target.dataset.index == bk_id) {
        if (e.target.dataset.index == wx.getStorageSync("bk_id")) {
          wx.seekBackgroundAudio({
            position: 1,
            success: function (res) {
              console.log(res)
      ...
      if (that.data.sign == 1) {
        ...
        console.log(66)
        console.log(wx.getStorageSync("bk_id"))
        console.log(e.target.dataset.index)
        if (e.target.dataset.index == wx.getStorageSync("bk_id")) {
          wx.seekBackgroundAudio({
    ...
    if (time == 0) {
      ...
      if (time > time1) {
        ...
        wx.request({
          url: 'https://www.chuang1.cn/api/bookrecord',
          method: "get",
          data: { 'bk_id': wx.getStorageSync("bk_id"), "tel": tel, "time": time, "type": 2 },
          success: function (res) {
            console.log(res)
          }
        ...
        wx.request({
          url: 'https://www.chuang1.cn/api/bookrecordd',
          method: "get",
          data: { 'bk_id': wx.getStorageSync("bk_id"), "tel": tel, "time": time },
          success: function (res) {
            console.log(res)
          }
  ...
  onLoad: function (options) {
    ...
    wx.getBackgroundAudioPlayerState({
      success: function (res) {
        ...
        if (res.status == 1) {
          if (options.bk_id == wx.getStorageSync("bk_id")) {
            that.setData({
              classes: "animationSlow"
            })
          }
    ...
    wx.request({
      url: 'https://www.chuang1.cn/api/bookdetail',
      method: 'get',
      data: { 'bk_id': options.bk_id, "tel": "'" + wx.getStorageSync("phone") + "'" },

      success: function (res) { 

        WxParse.wxParse("content", 'html', res.data.book.bk_content, that, 5)
        WxParse.wxParse("Test", 'html', res.data.book.bk_commend, that, 5)

        WxParse.wxParse("Test1", 'html', res.data.book.bk_abstract, that, 5)
        WxParse.wxParse("Dir", 'html', res.data.book.bk_dir, that, 5)
        ...
        /*if (res.data.sign == 2) { 
         console.log(787878) 
          ...
          console.log(89898)
          wx.setStorageSync("endtime", res.data.article.at_short)
          wx.setStorageSync("at_name", res.data.article.at_name)
          that.setData({
            data: res.data.article,
            name: res.data.article.at_name,
            music: res.data.article.at_shortmusic,
            stoptime: format(res.data.article.at_short),
            max: res.data.article.at_short,
            bk_id: options.bk_id,
            signs: res.data.sign,
            pic: res.data.article.bk_pic
          })
        }*/
        that.setData({
          data: res.data.book,
          name: res.data.book.bk_name,
          music: res.data.book.path,
          stoptime: format(res.data.book.time),
          max: res.data.book.time,
          bk_id: options.bk_id,
          signs: res.data.sign,
          pic: res.data.book.book_pic
        })
    ...
    if (wx.getStorageSync("bk_id") != options.bk_id) {
      ...
      if (options.type == 3) {
        ...
        wx.request({
          url: 'https://www.chuang1.cn/api/bookrecords',
          method: "get",
          data: { "tel": wx.getStorageSync("phone"), "bk_id": options.bk_id },
          success: function (res) {
